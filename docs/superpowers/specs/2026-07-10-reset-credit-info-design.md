...
- 每条明细提供 `id`、`resetType`、`status`、可选的 `title`，以及可选的 `expiresAt` Unix 时间戳。
...
每条 `ResetCredit` 保存展示所需的 `id`、`resetType`、`status`、可选标题和可选到期时间。协议状态和重置类型使用字符串，不定义封闭的 Swift 枚举，避免后端新增值时导致整个额度响应解析失败。
...
- 后端 `title` 非空时按原文显示，例如 `Full reset (Weekly + 5 hr)`。
- 后端没有标题且 `resetType == "codexRateLimits"` 时显示“完整额度重置”。
- 后端没有标题且重置类型未知时显示“额度重置”。
- 有到期时间时显示“将于 M/d 到期”。
- 没有到期时间时显示“无到期时间”。
...
新增独立的限额重置到期日期格式化逻辑，输出简短的 `M/d`。视图负责组合“将于……到期”和“无到期时间”。该逻辑不复用现有 5 小时与周限额重置时间格式化器，避免两类日期规则互相耦合。
...
- 后端标题缺失：根据重置类型使用“完整额度重置”或“额度重置”作为兜底。
...
- 可用次数、明细、标题、状态、重置类型和到期时间的解析。
- 字段缺失和 `null` 的情况。
- 旧缓存快照兼容性。
- 只筛选 `available` 明细，同时不改变 `availableCount`。
- 后端标题优先级，以及标题缺失时的兜底文案。
- `M/d` 到期日期及缺少到期时间的文案。
- 0 次和有次数但无明细的展示状态。
